...
bool CDVDDemuxFFmpeg::Open(const std::shared_ptr<CDVDInputStream>& pInput, bool fileinfo)
{
  ...
  else
  {
    ...
    std::string content = m_pInput->GetContent();
    KODI::StringUtils::ToLower(content);
    if (KODI::StringUtils::StartsWith(content, "audio/l16"))
...
AVDictionary* CDVDDemuxFFmpeg::GetFFMpegOptionsFromInput()
{
  ...
  if (input)
  {
        ...
        url.IsProtocol("rtmps"))
    {
      ...
      CURL tmpUrl = url;
      std::vector<std::string> opts = KODI::StringUtils::Split(tmpUrl.Get(), " ");
      if (opts.size() > 1) // inline rtmp options
      {
        ...
        {
          std::vector<std::string> value = KODI::StringUtils::Split(opts[i], "=", 2);
          KODI::StringUtils::ToLower(value[0]);
    ...
    while (pos < len)
    {
      ...
      if (pos != std::string::npos)
      {
        if (content.compare(pos, 9, "channels=", 9) == 0)
        {
          ...
          KODI::StringUtils::Trim(no_channels, " \t");
