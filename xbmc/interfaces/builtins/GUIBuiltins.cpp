...
static int AlarmClock(const std::vector<std::string>& params)
{
  ...
  if (params.size() > 2)
  {
    if (params[2].find(':') == std::string::npos)
      ...
      seconds = (float)KODI::StringUtils::TimeStringToSeconds(params[2]);
  }
  else
  { // check if shutdown is specified in particular, and get the time for it
    std::string strHeading;
    if (KODI::StringUtils::EqualsNoCase(params[0], "shutdowntimer"))
  ...
  for (unsigned int i = 3; i < params.size() ; i++)
  {
    ...
    if (KODI::StringUtils::EqualsNoCase(params[i], "true") || KODI::StringUtils::EqualsNoCase(params[i], "silent"))
      silent = true;
    else if (KODI::StringUtils::EqualsNoCase(params[i], "loop"))
      loop = true;
  }
...
static int CancelAlarm(const std::vector<std::string>& params)
{
  bool silent = (params.size() > 1 &&
      (KODI::StringUtils::EqualsNoCase(params[1], "true") ||
       KODI::StringUtils::EqualsNoCase(params[1], "silent")));
  g_alarmClock.Stop(params[0],silent);
...
{
  bool bForce = false;
  if (params.size() > 1 && KODI::StringUtils::EqualsNoCase(params[1], "true"))
    bForce = true;
  if (KODI::StringUtils::EqualsNoCase(params[0], "all"))
  {
...
static int Screenshot(const std::vector<std::string>& params)
{
  if (!params.empty())
  {
    ...
    bool sync = false;
    if (params.size() >= 2)
      sync = KODI::StringUtils::EqualsNoCase(params[1], "sync");
