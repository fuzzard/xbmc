...
static int CleanLibrary(const std::vector<std::string>& params)
{
  ...
  if (params.size() > 1)
    userInitiated = KODI::StringUtils::EqualsNoCase(params[1], "true");
  if (!params.size() || KODI::StringUtils::EqualsNoCase(params[0], "video")
                     || KODI::StringUtils::EqualsNoCase(params[0], "movies")
                     || KODI::StringUtils::EqualsNoCase(params[0], "tvshows")
                     || KODI::StringUtils::EqualsNoCase(params[0], "musicvideos"))
  {
  ...
  else if (KODI::StringUtils::EqualsNoCase(params[0], "music"))
  {
...
{
  int iHeading = 647;
  if (KODI::StringUtils::EqualsNoCase(params[0], "music"))
    iHeading = 20196;
  std::string path;
  VECSOURCES shares;
  ...
  if (params.size() > 1)
    singleFile = KODI::StringUtils::EqualsNoCase(params[1], "false");
  else
  {
  ...
  {
    if (params.size() > 2)
      thumbs = KODI::StringUtils::EqualsNoCase(params[2], "true");
    else
    {
  ...
  if (thumbs && !singleFile && KODI::StringUtils::EqualsNoCase(params[0], "video"))
  {
  ...
  if (thumbs && KODI::StringUtils::EqualsNoCase(params[0], "video"))
  {
    if (params.size() > 4)
      actorThumbs = KODI::StringUtils::EqualsNoCase(params[4], "true");
    else
    {
  ...
  {
    if (params.size() > 3)
      overwrite = KODI::StringUtils::EqualsNoCase(params[3], "true");
    else
    {
  ...
  if (!singleFile || !path.empty() ||
      CGUIDialogFileBrowser::ShowAndGetDirectory(shares, g_localizeStrings.Get(661),
                                                 path, true))
  {
    if (KODI::StringUtils::EqualsNoCase(params[0], "video"))
    {
...
static int ExportLibrary2(const std::vector<std::string>& params)
{
  ...
  if (KODI::StringUtils::EqualsNoCase(params[1], "separate"))
    settings.SetExportType(ELIBEXPORT_SEPARATEFILES);
  else if (KODI::StringUtils::EqualsNoCase(params[1], "library"))
  {
    settings.SetExportType(ELIBEXPORT_TOLIBRARYFOLDER);
  ...
  {
    if (KODI::StringUtils::EqualsNoCase(params[i], "artwork"))
      settings.m_artwork = true;
    else if (KODI::StringUtils::EqualsNoCase(params[i], "overwrite"))
      settings.m_overwrite = true;
    else if (KODI::StringUtils::EqualsNoCase(params[i], "unscraped"))
      settings.m_unscraped = true;
    else if (KODI::StringUtils::EqualsNoCase(params[i], "skipnfo"))
      settings.m_skipnfo = true;
    else if (KODI::StringUtils::EqualsNoCase(params[i], "albums"))
      settings.AddItem(ELIBEXPORT_ALBUMS);
    else if (KODI::StringUtils::EqualsNoCase(params[i], "albumartists"))
      settings.AddItem(ELIBEXPORT_ALBUMARTISTS);
    else if (KODI::StringUtils::EqualsNoCase(params[i], "songartists"))
      settings.AddItem(ELIBEXPORT_SONGARTISTS);
    else if (KODI::StringUtils::EqualsNoCase(params[i], "otherartists"))
      settings.AddItem(ELIBEXPORT_OTHERARTISTS);
    else if (KODI::StringUtils::EqualsNoCase(params[i], "actorthumbs"))
...
static int UpdateLibrary(const std::vector<std::string>& params)
{
  bool userInitiated = true;
  if (params.size() > 2)
    userInitiated = KODI::StringUtils::EqualsNoCase(params[2], "true");
  if (KODI::StringUtils::EqualsNoCase(params[0], "music"))
  {
    if (CMusicLibraryQueue::GetInstance().IsScanningLibrary())
  ...
  else if (KODI::StringUtils::EqualsNoCase(params[0], "video"))
  {
    if (CVideoLibraryQueue::GetInstance().IsScanningLibrary())
