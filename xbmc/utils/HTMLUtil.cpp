...
void CHTMLUtil::ConvertHTMLToW(const std::wstring& strHTML, std::wstring& strStripped)
{
  ...
  {
    KODI::StringUtils::Replace(strStripped, mappings[iPos].html,std::wstring(1, mappings[iPos].w));
    iPos++;
  }
  ...
  while (iPos > 0 && iPos < strStripped.size() - 4)
  {
    ...
    if (base == 10)
      num = KODI::StringUtils::Format(L"&#{};", num);
    else
      num = KODI::StringUtils::Format(L"&#x{};", num);

    KODI::StringUtils::Replace(strStripped, num,std::wstring(1,val));
