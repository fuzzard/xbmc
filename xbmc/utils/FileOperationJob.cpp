...
bool CFileOperationJob::CFileOperation::OnFileCallback(void* pContext, int ipercent, float avgSpeed)
{
  ...
  if (avgSpeed > 1000000.0f)
    data->base->m_avgSpeed = KODI::StringUtils::Format("{:.1f} MB/s", avgSpeed / 1000000.0f);
  else
    data->base->m_avgSpeed = KODI::StringUtils::Format("{:.1f} KB/s", avgSpeed / 1000.0f);

  std::string line;
  line =
      KODI::StringUtils::Format("{} ({})", data->base->GetCurrentFile(), data->base->GetAverageSpeed());
