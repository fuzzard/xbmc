...
void CPortManager::LoadXML()
{
  ...
  if (!pRootElement || pRootElement->NoChildren() ||
      strcmp(pRootElement->Value(), XML_ROOT_PORTS) != 0)
  {
...
{
  for (const auto* pPort = pElement->FirstChildElement(); pPort;
       pPort = pPort->NextSiblingElement())
  {
    if (strcmp(pPort->Value(), XML_ELM_PORT) != 0)
    {
...
void CPortManager::DeserializeControllers(const tinyxml2::XMLElement* pPort,
                                          ControllerNodeVec& controllers)
{
  for (const auto* pController = pPort->FirstChildElement(); pController;
       pController = pController->NextSiblingElement())
  {
    if (strcmp(pController->Value(), XML_ELM_CONTROLLER) != 0)
    {
...
void CPortManager::SerializePorts(tinyxml2::XMLElement& node, const PortVec& ports)
{
  ...
  {
    auto portNode = node.GetDocument()->NewElement(XML_ELM_PORT);
