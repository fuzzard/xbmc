                                    ...
                                    std::vector<CPhysicalFeature>& features)
{
  ...
  for (const auto* pChild = pElement->FirstChildElement(); pChild;
       pChild = pChild->NextSiblingElement())
  {
    if (strcmp(pChild->Value(), LAYOUT_XML_ELM_CATEGORY) == 0)
    {
      // Category
      ...
      // Features
      for (const auto* pFeature = pChild->FirstChildElement(); pFeature;
    ...
    else if (strcmp(pChild->Value(), LAYOUT_XML_ELM_TOPOLOGY) == 0)
    {
      // Topology
