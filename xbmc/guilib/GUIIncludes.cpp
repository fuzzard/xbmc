                                   ...
                                   std::map<INFO::InfoPtr, bool>* xmlIncludeConditions /* = NULL */)
{
  ...
  while (include)
  {
    ...
    if (it != m_includes.end())
    {
      ...
      tinyxml2::XMLNode* includeDefinitionChild = includeDefinitionDoc->FirstChildElement();
      while (includeDefinitionChild)
      {
        auto* insertedNode = node->InsertAfterChild(include, includeDefinitionChild);
                                ...
                                tinyxml2::XMLElement* node)
{
  ...
  if (nested)
  {
    ...
    while (child)
    {
      ...
      {
        target->InsertAfterChild(nested, child);
      }
      child = child->NextSiblingElement();
    }
