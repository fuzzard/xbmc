  ...
  if (reTitle.RegFind(strBuffer.c_str()) != -1)
  {
    ...
    if (result) // track has changed
    {
      ...
      if (haveStreamUrlData) // track has changed and extra metadata might be available
      {
        ...
        if (KODI::StringUtils::StartsWithNoCase(streamUrlData, "http://") ||
            KODI::StringUtils::StartsWithNoCase(streamUrlData, "https://"))
        {
          // Bauer Media Radio listenapi null event to erase current data
          if (!KODI::StringUtils::EndsWithNoCase(streamUrlData, "eventdata/-1"))
          {
            ...
            if (http.Get(dataURL.Get(), extData))
            {
              ...
              if (KODI::StringUtils::EqualsNoCase(contentType, "application/json"))
              {
                CVariant json;
        ...
        else if (KODI::StringUtils::StartsWithNoCase(streamUrlData, "&"))
        {
      ...
      if (artistInfo.empty() || title.empty())
      {
        ...
        const std::vector<std::string> tokens = KODI::StringUtils::Split(newtitle, " - ");
        if (tokens.size() == 2)
        {
          if (artistInfo.empty())
            artistInfo = tokens[0];
        ...
        else
        {
          if (title.empty())
          {
            ...
            if (!KODI::StringUtils::StartsWithNoCase(newtitle, "START ADBREAK ") &&
                !KODI::StringUtils::StartsWithNoCase(newtitle, "STOP ADBREAK "))
              title = newtitle;
          }
        }
      }
