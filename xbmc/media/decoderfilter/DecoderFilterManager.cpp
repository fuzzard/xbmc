...
bool CDecoderFilterManager::Load()
{
  ...
  {
    CLog::Log(LOGERROR, "{}: error loading: line {}, {}", CLASSNAME, xmlDoc.ErrorLineNum(),
              xmlDoc.ErrorStr());
    return false;
  }

  const auto* rootElement = xmlDoc.RootElement();
  if (!rootElement || !StringUtils::EqualsNoCase(rootElement->Value(), TAG_ROOT))
  {
    CLog::Log(LOGERROR, "{}: invalid root element ({})", CLASSNAME, rootElement->Value());
    return false;
  }

  const auto* pFilter = rootElement->FirstChildElement(TAG_FILTER);
  while (pFilter)
  {
