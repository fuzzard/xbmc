...
void CAdvancedSettings::ParseSettingsFile(const std::string &file)
{
  ...
  if (element)
  {
    ...
    XMLUtils::GetUInt(element, "maxpassthroughoffsyncduration", m_maxPassthroughOffSyncDuration, 10,
                      100);
  }
  ...
  if (element)
  {
    ...
    if (videoLatencyNode)
    {
      ...
      while (refreshVideoLatencyNode)
      {
        ...
        if (XMLUtils::GetFloat(refreshVideoLatencyNode, "delay", delay, -600.0f, 600.0f))
  ...
  element = rootElement->FirstChildElement("cache");
  if (element)
  {
    XMLUtils::GetUInt(element, "memorysize", m_cacheMemSize);
    XMLUtils::GetUInt(element, "buffermode", m_cacheBufferMode, 0, 4);
    XMLUtils::GetUInt(element, "chunksize", m_cacheChunkSize, 256, 1024 * 1024);
    XMLUtils::GetFloat(element, "readfactor", m_cacheReadFactor);
  }
