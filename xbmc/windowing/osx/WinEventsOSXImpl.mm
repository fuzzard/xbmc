...
- (instancetype)initWithName:(CWinEventsOSX*)initClass
{
  self = [super init];
...
{
  mRunLoopSource =
      CFMachPortCreateRunLoopSource(kCFAllocatorSystemDefault, (CFMachPortRef)mEventTap, 0);
  mRunLoop = CFRunLoopGetCurrent();
  CFRunLoopAddSource((CFRunLoopRef)mRunLoop, (CFRunLoopSourceRef)mRunLoopSource,
  ...
  if (mRunLoopSource)
  {
    CFRunLoopRemoveSource((CFRunLoopRef)mRunLoop, (CFRunLoopSourceRef)mRunLoopSource,
                          kCFRunLoopCommonModes);
...
void CWinEventsOSXImp::enableHotKeyTap()
{
  ...
  if (!m_TapThread->IsRunning())
  {
    ...
    mEventTap = CGEventTapCreate(kCGSessionEventTap, kCGHeadInsertEventTap,
                                 kCGEventTapOptionDefault, eventMask, HotKeyEventHandler, this);
    if (mEventTap != NULL)
    {
      m_TapThread->Create();
