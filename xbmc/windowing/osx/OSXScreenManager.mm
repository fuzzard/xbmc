...
//---------------------------------------------------------------------------------
CGDirectDisplayID COSXScreenManager::GetDisplayID(int screen_index)
{
...
size_t COSXScreenManager::DisplayBitsPerPixelForMode(CGDisplayModeRef mode)
{
...
CGDisplayModeRef COSXScreenManager::BestMatchForMode(
...
NSString* screenNameForDisplay(CGDirectDisplayID displayID)
{
  ...
  // IODisplayCreateInfoDictionary leaks IOCFUnserializeparse, nothing we can do about it.
      NSDictionary* deviceInfo = (__bridge NSDictionary*)IODisplayCreateInfoDictionary(
...
NSString* COSXScreenManager::screenNameForDisplay(CGDirectDisplayID displayID)
{
  @autoreleasepool
  {
...
int COSXScreenManager::GetDisplayIndex(std::string dispName)
{
  int ret = 0;
...
int COSXScreenManager::GetDisplayIndex(CGDirectDisplayID display)
{
  CGDirectDisplayID displayArray[MAX_DISPLAYS];
  CGDisplayCount numDisplays;
...
CGDisplayModeRef COSXScreenManager::GetMode(int width, int height, double refreshrate, int screenIdx)
{
...
void COSXScreenManager::UpdateResolutions()
{
  ...
  m_pWindowing->UpdateDesktopResolution2(
      CDisplaySettings::GetInstance().GetResolutionInfo(RES_DESKTOP), (dispName != nil) ? [dispName UTF8String] : "Unknown", w, h, fps, 0);

  if (dispName != nil)
  {
    CDisplaySettings::GetInstance().GetResolutionInfo(RES_DESKTOP).strOutput =
        [dispName UTF8String];
  }
  ...
  for (int i = 1; i < GetNumScreens(); i++)
  {
    ...
    m_pWindowing->UpdateDesktopResolution2(res, (dispName != nil) ? [dispName UTF8String] : "Unknown", w, h, fps, 0);
  ...
  for (int disp = 0; disp < numDisplays; disp++)
  {
    ...
    for (int i = 0; i < CFArrayGetCount(displayModes); ++i)
    {
          ...
          (interlaced == NO))
      {
        ...
        m_pWindowing->UpdateDesktopResolution2(res, (dispName != nil) ? [dispName UTF8String] : "Unknown", w, h, refreshrate, 0);
...
void COSXScreenManager::HandlePossibleRefreshrateChange()
{
  ...
  if (oldRefreshRate != m_refreshRate)
  {
    ...
    if (m_pAppWindow)
    {
      ...
      newEvent.resize.w = frame.size.width;
      newEvent.resize.h = frame.size.height;
