...
id objectFromVariant(const CVariant& data)
{
  ...
  if (data.isWideString())
    return [NSString stringWithCString:(const char*)data.asWideString().c_str()
                    ...
                    const CVariant& data)
{
  ...
  if (msg == "OnPlay" || msg == "OnResume")
  {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
      [g_xbmcController.DarwinEmbedMPNPInfoManager onPlay:item];
    });
  }
  else if (msg == "OnSpeedChanged" || msg == "OnPause")
  {
    ...
    [item setValue:@(g_application.GetTime()) forKey:@"elapsed"];
    dispatch_async(dispatch_get_main_queue(), ^{
      [g_xbmcController.DarwinEmbedMPNPInfoManager OnSpeedChanged:item];
    });
    if (msg == "OnPause")
    {
      dispatch_async(dispatch_get_main_queue(), ^{
        [g_xbmcController.DarwinEmbedMPNPInfoManager onPause:item];
      });
    }
  }
  else if (msg == "OnStop")
  {
    dispatch_async(dispatch_get_main_queue(), ^{
      [g_xbmcController.DarwinEmbedMPNPInfoManager onStop:dict[@"item"]];
    });
  }
}
