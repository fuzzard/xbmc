...
@synthesize DarwinEmbedMPNPInfoManager;
//--------------------------------------------------------------
...
- (id)initWithFrame:(CGRect)frame withScreen:(UIScreen *)screen
{
  ...
  DarwinEmbedMPNPInfoManager = [DarwinEmbedNowPlayingInfoManager new];
...
- (void)rescheduleNetworkAutoSuspend
{
  LOG(@"%s: playback state: %d", __PRETTY_FUNCTION__,  [DarwinEmbedMPNPInfoManager playbackState]);
  if ([DarwinEmbedMPNPInfoManager playbackState] == DARWINEMBED_PLAYBACK_PLAYING)
  {
    [self disableNetworkAutoSuspend];
    return;
  }
  ...
  int delay = [DarwinEmbedMPNPInfoManager playbackState] == DARWINEMBED_PLAYBACK_PAUSED ? 60 : 30;  // wait longer if paused than stopped
