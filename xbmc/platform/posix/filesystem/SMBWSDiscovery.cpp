...
{
  {
    CSingleLock lock(m_critWSD);
		...
		{
			std::string tmpxaddrs = item.xaddrs.substr((item.xaddrs.find(delim1) + delim1.size()));
			int found = tmpxaddrs.find(delim2);
...
std::string CWSDiscovery::GetBroadcastIP()
{
  ...
  case AF_INET:
    ...
    broadcast = ((struct sockaddr_in*)resIP->ai_addr)->sin_addr.s_addr | ~((struct sockaddr_in*)resnetmask->ai_addr)->sin_addr.s_addr;
