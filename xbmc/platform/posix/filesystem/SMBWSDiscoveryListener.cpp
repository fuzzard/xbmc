...
void CWSDiscoveryListenerUDP::Process()
{
  ...
  if (fd < 0)
  {
    CLog::Log(LOGDEBUG, "CWSDiscoveryListenerUDP::Process - Socket Creation failed");
    return;
  }
  ...
  {
    CLog::Log(LOGDEBUG, "CWSDiscoveryListenerUDP::Process - Reuse Option failed");
    Cleanup(true);
    return;
  }
  ...
  {
    CLog::Log(LOGDEBUG, "CWSDiscoveryListenerUDP::Process - Bind failed");
    Cleanup(true);
    return;
  }
  ...
  {
    CLog::Log(LOGDEBUG, "CWSDiscoveryListenerUDP::Process - Multicast Option failed");
    Cleanup(true);
    return;
  }
  ...
  {
    CLog::Log(LOGDEBUG, "CWSDiscoveryListenerUDP::Process - Loopback Disable Option failed");
    Cleanup(true);
    return;
  }
