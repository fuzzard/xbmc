...
private:
	...
	const std::string wsd_tag_find(const std::string& xml, const std::array<std::pair<std::string, std::string>, SIZE>& tag);

	void PrintWSDInfo(const WSDiscovery::wsd_req_info& info);

	const bool equalsAddress(const WSDiscovery::wsd_req_info& lhs, const WSDiscovery::wsd_req_info& rhs);
