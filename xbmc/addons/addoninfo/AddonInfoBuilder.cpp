                                 ...
                                 const RepositoryDirInfo& repo)
{
  ...
  if (_requires)
  {
    for (const auto* child = _requires->FirstChildElement("import"); child;
   ...
   * </extension>
   */
  for (const auto* child = element->FirstChildElement("extension"); child;
       child = child->NextSiblingElement("extension"))
  {
    ...
    if (point == "kodi.addon.metadata" || point == "xbmc.addon.metadata")
    {
      ...
      if (element)
      {
        for (const auto* elementsAssets = element->FirstChildElement(); elementsAssets;
                                          ...
                                          const tinyxml2::XMLElement* element)
{
  ...
  while (childElement)
  {
    ...
    if (!id.empty())
    {
      ...
      if (childElementText.empty())
      {
        const auto* childSubElement = childElement->FirstChildElement();
        if (childSubElement)
        {
                                    ...
                                    std::unordered_map<std::string, std::string>& translatedValues)
{
  ...
  for (const auto* child = element->FirstChildElement(tag.c_str()); child;
