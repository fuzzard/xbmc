                                         ...
                                         CFileItemList& items)
{
  for (int i=0;i<num_entries;++i)
  {
    ...
    {
      if (KODI::StringUtils::CompareNoCase(entries[i].properties[j].name, "propmisusepreformatted") == 0)
      {
        if (KODI::StringUtils::CompareNoCase(entries[i].properties[j].name, "true") == 0)
