...
if(NOT TARGET Python::PIL)
  ...
  macro(buildPythonmodulePIL)
    ...
    set(patches "${CMAKE_SOURCE_DIR}/tools/depends/target/${MODULE_LC}/10-win-cmake.patch"
                ...
                "${CMAKE_SOURCE_DIR}/tools/depends/target/${MODULE_LC}/pillow-crosscompile.patch")
    ...
    if(${CORE_SYSTEM_NAME} MATCHES "windows")
      ...
      find_package(Pythonmodule-Setuptools REQUIRED)
      ...
      # Todo: webos - PILPATHLIB=$(PYTHON_SITE_PKG)/..
      #       inspecting an ipk looks weird regarding location of PIL, not sure it should be required
      ...
      set(PYMOD_TARGETENV "AS=${CMAKE_AS}"
                          ...
                          "ZLIB_ROOT=${ZLIB_ROOT}"
                          "PYTHONPATH=${PIL_OUTPUT_DIR}:${PYTHON_SITE_PKG}"
    ...
    if(NOT ${CORE_SYSTEM_NAME} MATCHES "windows")
      # Extract egg file. This was(?) required as some platforms couldnt extract at runtime (ios/tvos?)
    ...
    # Setuptools only required for building for non windows platforms
  ...
  # Required, as we need it to set PYTHON_SITE_PKG
  ...
  if(PIL_VER VERSION_LESS ${${MODULE}_VER})
    ...
    add_dependencies(Python::PIL ${APP_NAME_LC}::Python)
    ...
    if(_multiconfig_generator)
      ...
      set_target_properties(${MODULE_LC} PROPERTIES EXCLUDE_FROM_ALL TRUE)
      add_dependencies(build_internal_depends ${MODULE_LC})
    endif()
  endif()
endif()
