...
function(core_add_library name)
  if(ENABLE_STATIC_LIBS)
    if(SOURCES)
      set(LIB_TYPE "STATIC")
      set(OPTION_TYPE "PUBLIC")
    else()
      set(LIB_TYPE "INTERFACE")
      set(OPTION_TYPE "INTERFACE")
    endif()
    add_library(${name} ${LIB_TYPE} ${SOURCES} ${HEADERS} ${OTHERS})
    ...
    core_target_link_libraries(${name} ${OPTION_TYPE})

    set(CORE_LIBRARY ${name} PARENT_SCOPE)

    if(NOT MSVC)
      target_compile_options(${name} ${OPTION_TYPE} ${CORE_COMPILE_OPTIONS})
    endif()
...
function(core_add_test_library name)
  if(ENABLE_STATIC_LIBS)
    ...
    add_library(${name} ${LIB_TYPE} ${SOURCES} ${SUPPORTED_SOURCES} ${HEADERS} ${OTHERS})
    set_target_properties(${name} PROPERTIES PREFIX ""
                                             EXCLUDE_FROM_ALL 1
...
function(core_target_link_libraries core_lib)
  ...
  foreach(_depspec ${required_deps})
    ...
    if(TARGET ${APP_NAME_LC}::${dep})
      target_link_libraries(${core_lib} ${INTERFACE_TYPE} ${APP_NAME_LC}::${dep})
    endif()
  endforeach()
