...
function convert2framework
{
  ...
  install_name_tool -change  @executable_path/Frameworks/${DYLIB_BASENAME} @executable_path/Frameworks/${DYLIB_LIBNAME}.framework/${DYLIB_LIBNAME} ${APP_FOLDER_PATH}/${PRODUCT_NAME}
  install_name_tool -add_rpath @executable_path/Frameworks/${DYLIB_LIBNAME}.framework ${APP_FOLDER_PATH}/${PRODUCT_NAME}

  BUNDLEID=`mdls -raw -name kMDItemCFBundleIdentifier ${APP_FOLDER_PATH}`
  if [ "${BUNDLEID}" == "(null)" ] ; then
    BUNDLEID=`/usr/libexec/PlistBuddy -c 'Print CFBundleIdentifier' ${APP_FOLDER_PATH}/Info.plist`
  fi
