...
# Do this in a post build step for the Kodi target
...
#  install_name_tool -add_rpath @executable_path/Frameworks/${DYLIB_LIBNAME}.framework ${TARGET_BUILD_DIR}/${EXECUTABLE_FOLDER_PATH}/${EXECUTABLE_NAME}
  ...
  # move for darwinembedded, copy for osx
  if [ "$PLATFORM_NAME" == "appletvos" ]; then
    mv -f "${DYLIB}" "${DEST_FRAMEWORK}/${DYLIB_LIBNAME}"
  elif [ "$PLATFORM_NAME" == "macosx" ]; then
    cp -f "${DYLIB}" "${DEST_FRAMEWORK}/${DYLIB_LIBNAME}"
  fi
...
if [ "$PLATFORM_NAME" == "appletvos" ]; then
  ...
  TARGET_FRAMEWORKS="${XBMC_DEPENDS}/Frameworks"
  for dylib in $(find "${XBMC_DEPENDS}/lib" -name "*.dylib" -type f -maxdepth 1); do
    convert2framework "${dylib}"
  done
fi
